...
class AppStore extends Component {
  ...
  getFilteredAppList = () => {
    ...
    const appsTab = appsList.filter(eachapp => eachapp.category === activeTabId)
    return appsTab
  }

  render() {
    const {searchInput, activeTabId} = this.state
    const {appsTab} = this.getFilteredAppList()
    const searchResults = appsTab.filter(each =>
          ...
          <div className="app-con">
            <ul>
              {searchResults.map(eachResult => (
                <AppItem key={eachResult.appId} appsDetails={eachResult} />
              ))}
            </ul>
          </div>
        </div>
